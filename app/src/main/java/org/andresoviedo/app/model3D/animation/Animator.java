...
public class Animator {
	...
	public static JointEngine jointengine;
...
//
	...
	private Map<String, float[]> getJoints(Map<String, float[]> temcurrentPose){
		...
		if(initialized == false && temcurrentPose != null){
			for (String key: temcurrentPose.keySet()) {
				init_map.put(key,temcurrentPose.get(key));

				initialized = true;


			}
			Log.d("JOINTS_LIST1",init_map.keySet().toString());
			//initialized = true;
	...
	private void applyPoseToJoints(Map<String, float[]> currentPose, Joint joint, float[] parentTransform) {
		...
		Log.d("TRANSFORM","Current Local Transform" + Arrays.toString(currentLocalTransform));
		Log.d("TRANSFORM","Animation Transform:" + Arrays.toString(animationTransform));
	}
